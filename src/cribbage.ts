...
export function initCribbage() {
  ...
  const playAgainButton = element<HTMLButtonElement>("#cribbage-play-again");
  const declineRematchButton = element<HTMLButtonElement>("#cribbage-decline-rematch");
  ...
  const state = {
    ...
    rematchVote: null as boolean | null,
    aiTimer: 0,
  ...
  const roundReadySeats = new Set<number>();
  const rematchVotes = new Map<number, boolean>();
  const rematchAiVotes = new Map<number, boolean>();
  const rematchAiAwaiting = new Set<number>();
  try {
  ...
  function renderNetworkList(games: Array<{ gameId: string; variant: Variant; playerCount: number; format: PlayFormat; seats: NetworkSeat[]; started: boolean; ownerSeat?: number; ownerName?: string }> = [], target: HTMLElement = availableGamesList, mine = false) {
    target.replaceChildren(...games.map((game) => { const row = document.createElement("div"); row.className = "cribbage-network-game"; const open = game.seats.filter((seat) => seat.available).length; const opponents = game.seats.filter((seat) => seat.control === "human" && seat.seat !== (game.ownerSeat ?? 0)); const connected = opponents.filter((seat) => seat.connected).length; const creator = mine ? "" : `Created by ${game.ownerName || "Player"} - `; row.innerHTML = `<div><strong>${game.gameId}</strong><span>${game.variant === "crazy" ? "Crazy" : "Standard"} - ${game.format === "team" ? "2 vs 2" : "Individual"} - ${creator}${mine ? `${connected}/${opponents.length} opponents connected` : `${open} open seat${open === 1 ? "" : "s"}`}${open ? ` - Waiting for ${open} player${open === 1 ? "" : "s"}` : ""}</span></div>`; if (!mine) { const enter = document.createElement("button"); enter.type = "button"; enter.textContent = "Join"; enter.disabled = game.started || open === 0; enter.addEventListener("click", () => { state.network.gameId = game.gameId; state.network.remote = true; networkSend({ type: "cribbage-join", gameId: game.gameId, userId: state.network.userId }); networkStatus.textContent = `Joining ${game.gameId}...`; }); row.append(enter); } else { const cancel = document.createElement("button"); cancel.type = "button"; cancel.className = "secondary"; cancel.textContent = "Cancel game"; cancel.addEventListener("click", () => { networkSend({ type: "cribbage-cancel", gameId: game.gameId, userId: state.network.userId }); networkStatus.textContent = `Cancelling ${game.gameId}...`; }); row.append(cancel); } return row; }));
  }
  ...
  function connectNetwork() {
    ...
    socket.addEventListener("message", (event) => { let message: any; try { message = JSON.parse(event.data); } catch { return; }
      ...
      if (message.type === "cribbage-rematch-ai-prompt" && !state.network.host) { askRematchAi(message.declinedSeats || []); return; }
      if (message.type === "cribbage-rematch-ended") { networkStatus.textContent = message.message || "Rematch ended."; roundMessage.textContent = networkStatus.textContent; return; }
  ...
  function applyNetworkSnapshot(snapshot: any, full = false) { if (!snapshot) return; state.phase = snapshot.phase; state.variant = snapshot.variant; state.format = snapshot.format; state.players = snapshot.players || []; const revealRound = snapshot.view === "round-over"; if (!full && !revealRound) state.players.forEach((player: Player, index: number) => { if (index !== state.network.seat) { player.hand = []; player.scoringHand = []; } }); state.deck = snapshot.deck || []; state.crib = snapshot.crib || []; state.cut = snapshot.cut || null; state.cutBonus = snapshot.cutBonus || 0; state.roundResults = snapshot.roundResults || []; state.dealer = snapshot.dealer; state.active = snapshot.active; state.total = snapshot.total; state.pegged = snapshot.pegged || []; state.pegHistory = snapshot.pegHistory || []; state.peggingCallouts = snapshot.peggingCallouts || []; state.round = snapshot.round; state.winner = snapshot.winner || ""; state.teamScores = snapshot.teamScores || { 1: 0, 2: 0 }; state.selected.clear(); state.nextRoundRequested = false; nextRoundButton.hidden = Boolean(state.winner); nextRoundButton.disabled = false; nextRoundButton.textContent = revealRound ? "Ready for next hand" : "Deal next round"; setPanels(revealRound ? "round-over" : "board"); renderBoard(); if (revealRound) { renderRoundResults(); showRematchChoices(); } }
  function humanSeatCount() { return state.players.filter((player) => player.control === "human").length; }
  function resetRematchUi() { state.rematchVote = null; playAgainButton.hidden = true; declineRematchButton.hidden = true; playAgainButton.disabled = false; declineRematchButton.disabled = false; rematchVotes.clear(); rematchAiVotes.clear(); rematchAiAwaiting.clear(); }
  function showRematchChoices() { const visible = Boolean(state.network.remote && state.winner); playAgainButton.hidden = !visible; declineRematchButton.hidden = !visible; playAgainButton.disabled = state.rematchVote !== null; declineRematchButton.disabled = state.rematchVote !== null; }
  function sendRematchEnded(message: string) { networkSend({ type: "cribbage-rematch-ended", message }); networkStatus.textContent = message; roundMessage.textContent = message; }
  function askRematchAi(declinedSeats: number[]) { const names = declinedSeats.map((seat) => state.players[seat]?.name || `Player ${seat + 1}`).join(", "); const accept = window.confirm(`${names} chose not to play again. Continue with AI in those seats?`); if (state.network.host) recordRematchAiVote(state.network.seat, accept); else networkSend({ type: "cribbage-action", action: { kind: "rematch-ai", accept } }); }
  function startRematch(aiSeats: number[] = []) { state.players = state.players.map((player, index) => ({ ...player, control: aiSeats.includes(index) ? "ai" : "human", score: 0, hand: [], scoringHand: [] })); state.winner = ""; state.round = 1; state.dealer = (state.dealer + 1) % state.players.length; state.teamScores = { 1: 0, 2: 0 }; resetRematchUi(); dealRound(); }
  function recordRematchAiVote(seat: number, accept: boolean) { if (!state.network.host || !rematchAiAwaiting.has(seat)) return; rematchAiVotes.set(seat, accept); if (rematchAiVotes.size < rematchAiAwaiting.size) return; const declined = [...rematchVotes.entries()].filter(([, vote]) => !vote).map(([index]) => index); if ([...rematchAiAwaiting].every((index) => rematchAiVotes.get(index))) startRematch(declined); else sendRematchEnded("Rematch cancelled. Everyone can return to the lobby."); }
  function recordRematchVote(seat: number, vote: boolean) { if (!state.network.host || state.winner === "") return; rematchVotes.set(seat, vote); if (rematchVotes.size < humanSeatCount()) return; const declined = [...rematchVotes.entries()].filter(([, value]) => !value).map(([index]) => index); if (!declined.length) { startRematch(); return; } rematchAiAwaiting.clear(); rematchAiVotes.clear(); for (const [index, value] of rematchVotes) if (value) rematchAiAwaiting.add(index); if (!rematchAiAwaiting.size) { sendRematchEnded("Rematch declined. Everyone can return to the lobby."); return; } const prompt = { type: "cribbage-rematch-ai-prompt", declinedSeats: declined }; for (const index of rematchAiAwaiting) { if (index === state.network.seat) askRematchAi(declined); else networkSend({ ...prompt, targetSeat: index }); } }
  function updateNextRoundStatus() { const ready = roundReadySeats.size; roundMessage.textContent = `Ready for next hand: ${ready} of ${humanSeatCount()} players. Everyone must confirm before the deal.`; nextRoundButton.disabled = state.nextRoundRequested; nextRoundButton.textContent = state.nextRoundRequested ? "Waiting for other players..." : "Ready for next hand"; }
  function advanceNetworkRoundIfReady() { if (!state.network.host || roundReadySeats.size < humanSeatCount()) return; roundReadySeats.clear(); state.nextRoundRequested = false; nextRoundButton.disabled = false; state.round += 1; state.dealer = (state.dealer + 1) % state.players.length; dealRound(); }
  function handleRemoteAction(seat: number, action: any) { if (action.kind === "rematch") { if (state.network.host) recordRematchVote(seat, Boolean(action.vote)); return; } if (action.kind === "rematch-ai") { if (state.network.host) recordRematchAiVote(seat, Boolean(action.accept)); return; } if (action.kind === "next-round") { if (state.network.host && state.view === "round-over" && !state.winner && state.players[seat]?.control === "human") { roundReadySeats.add(seat); updateNextRoundStatus(); advanceNetworkRoundIfReady(); } return; } if (seat !== state.active || state.players[seat]?.control !== "human") return; state.selected = new Set(action.selected || []); if (action.kind === "discard") applyDiscard(true); else if (action.kind === "play") { const card = state.players[seat].hand.find((candidate) => candidate.id === action.cardId); if (card) playCard(card, true); } else if (action.kind === "pass") passPegging(true); }
  ...
  function finishRound() {
    ...
    nextRoundButton.textContent = state.network.remote ? "Ready for next hand" : "Deal next round";
    state.nextRoundRequested = false;
    roundReadySeats.clear();
    showRematchChoices();
